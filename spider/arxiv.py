...
def get_abstract(driver, recent_papers, domains=['cs.CV'], trans2zh_CN=False):
    print('get abstracts...')
    for domain in domains:
        print(domain)
...
if __name__ == '__main__':
    ...
    # obtain papers
    domains = ['cs.CV','cs.AI']
    cn_url = True # if use CN link i.e http://cn.arxiv.org
    abstract = True
    trans2zh_CN = True # translate abstract
    recent_papers = get_arxiv_recent_domain_papers(driver, domains=['cs.CV'], \
                                                   trans2zh_CN=trans2zh_CN,\
                                                   wait_time=10,cn_url=cn_url,abstract=abstract)

    # save to file
    current_date_time = time.strftime("%Y_%m_%d_%Hh%m", time.localtime())
